...
SELECT tmp1.campus, Agriculture, Biology
...
where disciplines.Name = 'Agriculture' and discEnr.gr > 0 and discEnr.year = 2004) as tmp1;
...
SELECT firstFlight.FlightNo, secondFlight.FlightNo, secondFlight.Destination, airports.name as Code from (SELECT * from flights
join airlines on flights.Airline = airlines.id
where airlines.Abbr = "Northwest" and flights.Source = "AXX") as firstFlight
join (SELECT * from flights
join airlines on flights.Airline = airlines.id
where airlines.Abbr = "Northwest" and flights.Destination != "AXX") as secondFlight on firstFlight.Destination = secondFlight.Source
join airports on secondFlight.Destination = airports.code;
