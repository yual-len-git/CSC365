...
select flights.source, airports.name from flights
...
count(distinct flights.Source) as Airports from airlines
...
select goods.Flavor, round(avg(goods.price),2) as AveragePrice, count(*) as DifferentPastries from goods
...
select sum(goods.price) from items
...
select receipts.RNumber, receipts.SaleDate, count(*) as NumberOfItems, round(sum(goods.Price),2) as CheckAmount from customers
...
select dayname(receipts.SaleDate) as Day, receipts.SaleDate, count(distinct items.Receipt) as Receipts, count(*) as Items, round(sum(goods.Price), 2) as Revenue from receipts
join items on items.Receipt = receipts.RNumber
join goods on goods.GId = items.Item
...
select receipts.SaleDate from receipts
join items on items.Receipt = receipts.RNumber 
join goods on goods.GId = items.Item
where food = "Tart"
...
select campuses.campus, avg(enrollments.Enrolled) as average, min(enrollments.Enrolled) as minimum, max(enrollments.Enrolled) as maximum from campuses 
...
Select rooms.RoomName, sum(datediff(reservations.Checkout, reservations.CheckIn) * reservations.Rate) as TotalRevenue, round(avg(datediff(reservations.Checkout, reservations.CheckIn) * reservations.Rate), 2) as AverageStay from rooms
...
Select count(*) as Stays, sum(reservations.Rate * datediff(reservations.CheckOut, reservations.CheckIn)) as Revenue from rooms
join reservations on reservations.Room = rooms.RoomCode 
...
Select dayname(reservations.CheckIn) as Day, count(*) as Stays, sum(datediff(reservations.CheckOut, reservations.CheckIn) * reservations.Rate) as revenue from rooms
join reservations on reservations.Room = rooms.RoomCode 
group by Day
...
Select rooms.roomname, max(reservations.rate - rooms.basePrice) as Markup, min(reservations.rate - rooms.basePrice) as Discount from rooms
join reservations on reservations.Room = rooms.RoomCode
group by rooms.roomname
...
from(select rooms.RoomCode, rooms.RoomName,
    case
    ...
    end as Days from rooms
    join reservations on reservations.Room = rooms.RoomCode
    where (Year(reservations.CheckIn) <= 2010 and Year(reservations.Checkout) >= 2010) or (Year(reservations.CheckIn) <= 2010 and Year(reservations.Checkout) >= 2011)) as r1
...
select Band.Firstname, count(*) from Vocals
...
select Band.Firstname, count(distinct Instruments.Instrument) from Instruments
...
select Performance.StagePosition as TuridPosition, count(*) as 'count' from Performance
...
from(select distinct Performance.Song as Song, Band.Firstname, Instrument from Performance
    join Instruments on  Instruments.Song = Performance.Song
    join Band on Band.Id = Instruments.Bandmate
    where Band.Firstname != 'Anne-Marit' and Instruments.Instrument = 'bass balalaika') as k1
    
join(select Performance.Song as Song from Performance
    join Band on Band.Id = Performance.Bandmate
    where Performance.StagePosition = 'left' and Band.Firstname  = 'Anne-Marit') as k2
    on k1.Song = k2.Song
...
select Band.Firstname, count(*) from Band
join(select Instruments.Bandmate from Instruments
    group by Instruments.Bandmate, Instruments.Song
    having count(Instruments.Instrument) > 1) as k1
    on Band.Id = Bandmate
...
select marathon.AgeGroup, marathon.Sex, count(*) as Total, min(marathon.Place) as BestPlacing, max(marathon.Place) as SlowestPacing from marathon 
...
select count(*)
from (select count(*) from marathon 
    where GroupPlace < 3
...
select minute(marathon.pace), count(*) as 'count' from marathon
...
select distinct marathon.State, count(*) over(partition by marathon.State) as NumberOfTop10 from marathon
where GroupPlace < 11 
...
select marathon.Town, round(avg(time_to_sec(marathon.RunTime)), 1) as AverageTimeInSeconds from marathon
...
select list.grade, count(distinct list.classroom) as Classrooms, count(*) as Students from list
group by list.grade
...
select list.classroom, count(*) as total from list
where list.grade = 0
...
select list.classroom, max(list.Lastname) as LastOnRoster from list
where list.grade = 4
group by list.classroom;
